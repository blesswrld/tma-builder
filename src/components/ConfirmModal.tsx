    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="relative w-full max-w-md bg-app-surface border border-app-border rounded-2xl p-5 sm:p-6 shadow-2xl space-y-4">
        ...
        <button
          onClick={onClose}
          disabled={isLoading}
          className="absolute top-4 right-4 text-app-muted hover:text-app-text-primary p-1 rounded-lg transition-colors cursor-pointer"
        >
          <X size={18} />
        </button>
        ...
        <div className="flex items-start gap-3.5">
          ...
          <div className="space-y-1 pr-4">
            <h3 className="text-base font-semibold text-app-text-primary leading-tight">
              {title}
            </h3>
            <div className="text-xs text-app-text-secondary leading-relaxed pt-1">
              {message}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-app-border/50">
          <button
            ...
            onClick={onClose}
            disabled={isLoading}
            className="px-4 py-2 bg-app-card hover:bg-app-bg border border-app-border text-app-text-secondary hover:text-app-text-primary text-xs font-medium rounded-xl transition-all cursor-pointer disabled:opacity-50"
          >
            {cancelText}
          </button>
          <button
