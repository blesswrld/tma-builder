        ...
        <div key="my-orders-modal-container" className="fixed inset-0 z-50">
          ...
          >
            <div className="h-16 flex items-center justify-between px-6 border-b border-app-border bg-app-modal-header">
              ...
              <button onClick={onClose} className="text-app-muted hover:text-app-primary transition-colors cursor-pointer">
                <X size={18} />
              </button>
            </div>
                  ...
                  <div key={order.id} className="p-4 border border-app-border rounded-2xl bg-app-card space-y-3">
                    <div className="flex justify-between items-center text-xs font-mono">
                      ...
                      <span className={`px-2 py-0.5 rounded-md text-[10px] uppercase font-bold border ${
                        order.status === 'COMPLETED' ? 'bg-app-accent text-app-accent-fg border-transparent' :
                    ...
                    <button onClick={() => onReorder(order)} className="w-full py-2 bg-app-secondary hover:bg-app-hover rounded-xl text-xs font-mono text-app-primary transition-colors cursor-pointer">
                      Повторить заказ
                    </button>
                  </div>
                ))
